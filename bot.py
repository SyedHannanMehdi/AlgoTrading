...
“”“POST with params in query string — for /fapi/v1/order and most endpoints.”””
r = requests.post(BASE_URL + path, headers=_headers(), params=_sign(params), timeout=10)
return r.json()

def b_post_body(path, params):
“”“POST with params in request body (form-encoded) — required for /fapi/v1/algoOrder.”””
signed = _sign(params)
r = requests.post(BASE_URL + path, headers=_headers(), data=signed, timeout=10)
print(f”[HTTP] POST {path} status={r.status_code} body={r.text[:500]}”)
return r.json()
...
```
if tp_price > 0:
    r = b_post_body("/fapi/v1/algoOrder", {
        "algoType":    "CONDITIONAL",
        "symbol":      symbol,
        "side":        close_side,
        "type":        "TAKE_PROFIT_MARKET",
        "triggerPrice": round_price(tp_price, tick),
        "closePosition": "true",
        "timeInForce": "GTC",
        "workingType": "MARK_PRICE",
        "clientAlgoId": uuid.uuid4().hex[:36],
    })
...
if sl_price > 0:
    r = b_post_body("/fapi/v1/algoOrder", {
        ...
        "side":        close_side,
        "type":        "STOP_MARKET",
        "triggerPrice": round_price(sl_price, tick),
        "closePosition": "true",
        "timeInForce": "GTC",
        "workingType": "MARK_PRICE",
        "clientAlgoId": uuid.uuid4().hex[:36],
